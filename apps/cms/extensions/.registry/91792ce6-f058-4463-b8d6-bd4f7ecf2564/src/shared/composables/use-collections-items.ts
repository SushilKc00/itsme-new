...
export function useCollectionsItems() {
  ...
  const collections = computed(() => {
    ...
    return collections
      .filter((collection: any) => {
         ...
         */
        return (
          excludeCollection(collection.collection) &&
      ...
      ?.map((collection: any) => {
        ...
        const translationField = fieldsStore
          .getFieldsForCollection(collection.collection)
          .find((field: any) => field.meta.interface === 'translations');

        if (!translationField) {
          return collection;
        }

        const relationTranslation = relations.find(
          (relation: any) => relation.meta.one_field === translationField.field
        );
  ...
  const collectionsWithoutTranslation = computed(() => {
    return collections.value.filter(
      (collection: any) => !listTranslationCollections.value?.includes(collection.collection)
    );
  });

  const translationCollections = computed(() => {
    return collections.value.filter((collection: any) =>
      listTranslationCollections.value?.includes(collection.collection)
    );
  });
