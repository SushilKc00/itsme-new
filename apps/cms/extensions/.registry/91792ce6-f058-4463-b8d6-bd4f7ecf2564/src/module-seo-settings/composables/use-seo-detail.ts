...
export function useSEODetail(collection: Ref<string>) {
  ...
  async function createField() {
    ...
    return await fieldsStore.createField(
      targetCollection.value,
      getSeoDetailsField(targetCollection.value)
    );
  }
  async function createRelation() {
    const existing = relationsStore.getRelationForField(
      targetCollection.value,
    ...
    return await relationsStore.upsertRelation(
      targetCollection.value,
      COLLECTION.seo_detail,
      getSeoDetailRelation(targetCollection.value)
    );
  }
