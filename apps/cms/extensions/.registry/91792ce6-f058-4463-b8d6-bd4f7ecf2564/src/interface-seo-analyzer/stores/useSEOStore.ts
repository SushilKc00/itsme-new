...
export const useSEOStore = defineStore('SEOStore', () => {
  ...
  const getPaper = (keyword: string, keywords: string[]) => {
    ...
    paper.setText(Helpers.unescape(content.value));
    ...
    if (!isUndefined(options.value.thumbnail)) {
      ...
      paper.setThumbnailAltText(Helpers.removeDiacritics(options.value.thumbnailAlt));
    }
  ...
  const analyzeScore = () => {
    ...
    keywords.map((keyword, index) => {
      const paper = getPaper(Helpers.removeDiacritics(keyword), keywords);
      ...
      analyzer.value.analyze(paper).then((analyzerResults: any) => {
        resultManager.value.update(paper.getKeyword(), analyzerResults, 0 === index);
      });
    });
  };
